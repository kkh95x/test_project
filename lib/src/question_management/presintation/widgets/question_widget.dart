        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: Text('Reputation: ${question.owner.reputation ?? 0}'),
            ),
            ...
            Row(
              ...
              children: [
                _buildInfoRow(
                    Icons.remove_red_eye, '${question.viewCount} views'),
                _buildInfoRow(
                    Icons.question_answer, '${question.answerCount} answers'),
                _buildInfoRow(Icons.thumb_up, '${question.score} score'),
              ],
            ),
            const SizedBox(height: 10.0),
            Text(
              'Last activity: ${_formatDate(question.lastActivityDate)}',
              style: const TextStyle(color: Colors.grey),
            ),
            Text(
              'Created on: ${_formatDate(question.creationDate)}',
              style: const TextStyle(color: Colors.grey),
            ),
            if (question.lastEditDate != null)
              Text(
                'Last edited: ${_formatDate(question.lastEditDate!)}',
                style: const TextStyle(color: Colors.grey),
              ),
          ],
        ),
